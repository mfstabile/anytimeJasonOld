...
public class TransitionSystem {
    ...
    public boolean reasoningCycle() {
        ...
        try {
            ...
            //print nrcslbr and nrcbp and find out wth does it means
...
//Add Another timer
long tHalf = System.currentTimeMillis();
                getUserAgArch().checkMail();
//End timer   
long tEnd = System.currentTimeMillis();
long tPercept = tHalf - tStart;
long tMail = tEnd - tHalf;

//Save timer inside text file
try {
    PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("time visualizer.txt", true)));
    out.println("Perceipt time:" + tPercept);
    out.println("Message time:" + tMail);
    out.close();
} catch (IOException e) {
...
try {
    PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("time visualizer.txt", true)));
    out.println("nrcslbr:" + nrcslbr);
    out.println("setts.nrcbp():" + setts.nrcbp());
    out.close();
} catch (IOException e) {
    //exception handling left as an exercise for the reader
}            
