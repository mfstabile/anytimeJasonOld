...
public class TransitionSystem {
    ...
    private void applyRelPl() throws JasonException {
        ...
        // Rule Rel1
        if (confP.C.RP != null || setts.retrieve()) // retrieve is mainly for Coo-AgentSpeak
        {
    ...
    private void applyApplPl() throws JasonException {
        ...
        // Rule Appl1
        if (confP.C.AP != null || setts.retrieve()) // retrieve is mainly for Coo-AgentSpeak
        {
    ...
    public void applyClrInt(Intention i) throws JasonException {
        while (true) { // quit the method by return
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine("Returning from IM " + topIM.getPlan().getLabel() + ", te=" + topIM.getPlan().getTrigger() + " unif=" + topIM.unif);
            }
    ...
    public boolean reasoningCycle() {
        System.out.println("Begin of cycle: " + ag.toString());
        if (logger.isLoggable(Level.FINE)) {
        ...
        try {
            ...
            if (nrcslbr >= setts.nrcbp()) {
                nrcslbr = 0;
                System.out.println("Perceived: " + ag.toString());
                long startTime = System.currentTimeMillis();
                ag.buf(getUserAgArch().perceive());
                //Thread.sleep(100);
                long stopTime = System.currentTimeMillis();
                long elapsedTime = stopTime - startTime;
                System.out.println("Time for buf: " + elapsedTime);
